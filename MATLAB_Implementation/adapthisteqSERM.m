function out = adapthisteqSERM(varargin)

% This function is edited from MATLAB's function adapthisteq
% by Md Tauhidul Islam for using in SERM. 'Gaussian' distribution has been
% added. The name has been changed to prevent any conflict

%adapthisteqSERM Contrast-limited Adaptive Histogram Equalization (CLAHE).
%   adapthisteqSERM enhances the contrast of images by transforming the
...
%
%   J = adapthisteqSERM(I) Performs CLAHE on the intensity image I.
%
%   J = adapthisteqSERM(I,PARAM1,VAL1,PARAM2,VAL2...) sets various parameters.
...
%
%      I = imread('tire.tif');
%      A = adapthisteqSERM(I,'clipLimit',0.02,'Distribution','rayleigh');
%      figure
%      montage({I,A});
%
%   Example 2
...
switch distribution
 ...
 case 'rayleigh', % suitable for underwater imagery
  ...
  val = vmax*(histSum/numPixInTile);
  val(val>=1) = 1-eps; % avoid log(0)
  temp = sqrt(-hconst*log(1-val));temp=rescale(temp);
  mapping = min(selectedRange(1)+temp*valSpread,...
                selectedRange(2)); %limit to max
...
for k=1:numTiles(1)+1
  ...
  for l=1:numTiles(2)+1
    ...
    claheI(imgTileIdx{1}, imgTileIdx{2}) = ...
        (rowRevW .* (colRevW .* double(images.internal.builtins.grayxform(imgPixVals,ulMapTile)) + ...
                     colW    .* double(images.internal.builtins.grayxform(imgPixVals,urMapTile)))+ ...
         rowW    .* (colRevW .* double(images.internal.builtins.grayxform(imgPixVals,blMapTile)) + ...
                     colW    .* double(images.internal.builtins.grayxform(imgPixVals,brMapTile))))...
        /normFactor;
...
if nargin > 1
  ...
  while ~done
    ...
    switch inputStr
     ...
     case 'Distribution'
      validDist = {'rayleigh','gaussian','exponential','uniform'};
